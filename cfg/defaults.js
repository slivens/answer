...
const _getMomentLang = require('./script/get-moment-lang')
...
const getheme = require('./script/getheme') // 获取less变量的覆盖值
const BundleAnalyzerPlugin = require('webpack-bundle-analyzer').BundleAnalyzerPlugin // 分析文件构成
...
/** 合并的外部包 **/
const vendorJs = ['react', 'react-router', 'redux', 'react-redux', 'redux-actions', 'react-intl', 'axios']
...
const alias = {
  ...
  'react-css-modules': '@sdp.nd/react-css-modules',
  fish: '@sdp.nd/fish'
}
// END ====================================================================================================
...
const fileName = isDist ? staticPath + '/[name]-[chunkhash].js' : staticPath + '/[name]-[hash].js'
...
const stylesPathAll = [srcPath, nodeModulesPath] // 源码样式路径,node_modules路径
...
/** 样式提取配置 **/
const extractCSS = new ExtractTextPlugin(staticPath + '/[name]-[chunkhash].css', { allChunks: true })
...
const defaultModules = {
  ...
  loaders: [
    ...
    },
    {
      ...
      loader: extractCSS.extract(
        'style-loader',
        'css-loader?modules&importLoaders=1&localIdentName=[name]__[local]-[hash:base64:5]!postcss-loader!sass-loader?outputStyle=expanded'
      )
    },
    {
      test: /\.scss/,
      include: stylesPathAll,
      loader: extractCSS.extract('style-loader', 'css-loader!postcss-loader!sass-loader?outputStyle=expanded')
    },
    {
      test: /\.less/,
      exclude: stylesPathAll,
    ...
    },
    {
      ...
      loader: extractCSS.extract(
        'style-loader',
        'css-loader!postcss-loader!less-loader?{"modifyVars":' + getheme(pkg.theme) + '}'
      )
    },
    {
      test: /\.styl/,
      loader: extractCSS.extract('style-loader', 'css-loader!stylus-loader!postcss-loader')
    },
    {
...
const LANG_LIST = pkg.needFac ? (function (data) {
  ...
  langList.forEach(function (item, index) {
    ...
    if (momentLang.indexOf(_name) !== -1) {
      _exp.push(_name)
    } else if (_name.indexOf('-') !== -1 && momentLang.indexOf(_name.split('-')[0]) !== -1) {
      _exp.push(_name.split('-')[0])
    }
  })
  return new RegExp(_exp.join('|'))
})(_fs.existsSync(langListPath) ? require(langListPath) : {langList: []}) : new RegExp('zh-cn')
